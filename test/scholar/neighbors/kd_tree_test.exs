defmodule Scholar.Neighbors.KDTreeTest do
  ...
  describe "unbounded" do
    test "sample" do
      assert %KDTree{levels: 4, indices: indices} =
               KDTree.unbounded(example(), compiler: EXLA)
    ...
    test "float" do
      assert %KDTree{levels: 4, indices: indices} =
               KDTree.unbounded(example() |> Nx.as_type(:f32),
                 compiler: EXLA
               )
    ...
    test "corner cases" do
      assert %KDTree{levels: 1, indices: indices} =
               KDTree.unbounded(Nx.iota({1, 2}), compiler: EXLA)

      assert indices == Nx.u32([0])

      assert %KDTree{levels: 2, indices: indices} =
               KDTree.unbounded(Nx.iota({2, 2}), compiler: EXLA)
  ...
  describe "bounded" do
    ...
    test "float" do
      assert %KDTree{levels: 4, indices: indices} =
               KDTree.bounded(example() |> Nx.as_type(:f32), 100)
  ...
  describe "query" do
    test "all defaults" do
      kdtree = KDTree.bounded(x(), 10)

      assert KDTree.query(kdtree, x_pred()) ==
    ...
    test "metric set to {:minkowski, 1.5}" do
      ...
      assert KDTree.query(kdtree, x_pred(), metric: {:minkowski, 1.5}) ==
    ...
    test "k set to 4" do
      ...
      assert KDTree.query(kdtree, x_pred(), k: 4) ==
