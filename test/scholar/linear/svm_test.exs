defmodule Scholar.Linear.SVMTest do
  ...
  test "test column target" do
    ...
    col_model = SVM.fit(x_train, y_train |> Nx.new_axis(-1),
                    num_classes: 3, loss_fn: loss_fn)
  ...
  test "test fit 2 columned y data" do
    ...
    y_train = Nx.concatenate(
                [y_train |> Nx.new_axis(-1), y_train |> Nx.new_axis(-1)],
                axis: 1
              )
    {n_samples, _} = Nx.shape(x_train)
    message = "Elixir.#{inspect(SVM)} expected y to have shape #{n_samples}, got tensor with shape: #{inspect(Nx.shape(y_train))}"
    assert_raise ArgumentError,
                 message, 
                 fn -> SVM.fit(x_train, y_train, num_classes: 3, loss_fn: loss_fn)
                 end
  end  
