defmodule Scholar.Covariance.LedoitWolfTest do
  ...
  defp x do
    key = key()
    {x, _new_key} = Nx.Random.multivariate_normal(key, Nx.tensor([0.0, 0.0, 0.0]), Nx.tensor([[3.0, 2.0, 1.0], [1.0, 2.0, 3.0], [1.3, 1.0, 2.2]]), shape: {10}, type: :f32)
    x
  end

  defp x2 do
    key = key()
    {x, _new_key} = Nx.Random.multivariate_normal(key, Nx.tensor([0.0, 0.0]), Nx.tensor([[2.2, 1.5], [0.7, 1.1]]), shape: {50}, type: :f32)
    x
  end

  defp x3 do
    key = key()
    {x, _new_key} = Nx.Random.multivariate_normal(key, Nx.tensor([0.0]), Nx.tensor([[0.4]]), shape: {15}, type: :f32)
    x |> Nx.flatten()
  end
  ...
  test "fit test - set :block_size" do
    model = LedoitWolf.fit(x2(), block_size: 20)
    ...
    assert_all_close(model.location, Nx.tensor([0.06882287561893463, 0.13750331103801727]), atol: 1.0e-3)
  end

  test "fit test - 1 dim x" do
    model = LedoitWolf.fit(x3())
    assert_all_close(
      model.covariance,
      Nx.tensor([
        [0.5322133302688599]
