defmodule Scholar.Integrate.Simpson do
  ...
  @simpson_schema NimbleOptions.new!(general)
  ...
  ## Examples
      ...
      iex> Scholar.Integrate.Simpson.simpson(Nx.tensor([1, 2, 3]), Nx.tensor([4, 5, 6]))
      #Nx.Tensor<
        f32
        4.0
      >

      iex> Scholar.Integrate.Simpson.simpson(Nx.tensor([[0, 1, 2], [3, 4, 5]]), Nx.tensor([[1, 2, 3], [1, 2, 3]]))
      #Nx.Tensor<
        f32[2]
        [2.0, 8.0]
      >

      iex> Scholar.Integrate.Simpson.simpson(Nx.tensor([[0, 1, 2], [3, 4, 5]]), Nx.tensor([[1, 1, 1], [2, 2, 2]]), axis: 0)
      #Nx.Tensor<
        f32[3]
        [1.5, 2.5, 3.5]
      >
  ...
  defnp simpson_n(y, x, opts \\ []) do
    ...
    x =
      if Nx.rank(x) == 1 do
        shape = prepare_shape(y_ndim, Nx.shape(x), axis)
        Nx.reshape(x, shape)
      else
        x
      end
  ...
  ## Examples
      ...
      iex> Scholar.Integrate.Simpson.simpson_uniform(Nx.tensor([1, 2, 3]))
      #Nx.Tensor<
        f32
        4.0
      >

      iex> Scholar.Integrate.Simpson.simpson_uniform(Nx.tensor([1, 2, 3]), dx: 2)
      #Nx.Tensor<
        f32
        8.0
      >

      iex> Scholar.Integrate.Simpson.simpson_uniform(Nx.tensor([[0, 1, 2], [3, 4, 5]]), dx: 2, axis: 0)
      #Nx.Tensor<
        f32[3]
        [3.0, 5.0, 7.0]
      >
  ...
  deftransform simpson_uniform(y, opts \\ []) do
    simpson_uniform_n(y, NimbleOptions.validate!(opts, @uniform_schema))
  end
